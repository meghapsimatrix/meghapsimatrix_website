...
The mean of the distribution is `r round(mean(heights$heights), 3)` and the standard deviation of the distribution is `r round(sd(heights$heights), 3)`. These are the population *parameters*:
...
The mean of the sample of heights is `r round(mean(sample_1$heights), 3)` and the standard deviation is `r round(sd(sample_1$heights), 3)`. This is the sample mean and the sample sd, the sample *statistics*. The sample mean is not going to be exactly the same as the population parameter and will differ from sample to sample.
